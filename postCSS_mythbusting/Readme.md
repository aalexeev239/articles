...
С самого начала PostCSS поставил перед собой интересную задачу. На фоне его соперничества с такими обработчиками CSS, как Sass и Less, появились некоторые заблуждения, связанные с тем, как его воспринимают и используют.
...
PostCSS подходит для парсинга как CSS, так и SCSS синтаксиса, а это значит, что вы можете использовать PostCSS как до, так и после компиляции Sass. Например, на текущем проекте я использую PostCSS чтобы прогнать свои Sass файлы через Stylelint перед тем, как компилировать их в CSS. А после этого получившийся CSS расширяется плагинами вроде Autoprefixer и postcss-assets, которые добавляют вендорные префиксы и встраивают графику через data URIs. Так что ваш рабочий процесс может выглядеть примерно так:

![using postcss](http://dab1nmslvvntp.cloudfront.net/wp-content/uploads/2016/03/1457051087workflow.jpg)
...
Если по каким-либо причинам вы не используете Autoprefixer, посмотрите на другие доступные плагины. Каждый проект и каждая команда разработчиков отличаются друг от друга, и даже если вы просто пролистаете список доступных плагинов, вы сможете найди там что-то полезное для вас и для вашей команды.
...
Если вас все устраивает в текущем препроцессоре и раньше вы не использовали PostCSS, поскольку он предлагал всё то же самое, стоит взглянуть на него ещё раз. К тому же, такие плагины, как Stylelint, PostCSS Stats и doiuse могут обеспечить автоматический анализ вашего CSS, чего не предложит стандартный препроцессор.

Также можно оптимировать код, например, минифицировать SVG и CSS, предоставить более изящные способы добавления фоллбэков для стилей, чем миксины.

Если вы хотите поэкспериментировать с CSS модулями, то и тут у PostCSS найдется подходящий плагин.
